...
Note: The analyses on C&C '26 poster start at 'H1 and H2: Compliant Analyses.'
...
Most participants in the control and high-agency conditions recalled the instructions, but only 56% of participants in the low-agency condition correctly recalled the instructions.
```{r}
...
```{r}
paste_events <- iriss_trial_data |> 
  filter(condition != "Control") |>
  select(submitter_id,
         condition,
         use,
         ends_with("pasteEvents",
                   ignore.case = FALSE))

write_csv(paste_events, "../../data/paste_events.csv")
```


```{r}
paste_events <- read_csv("../../data/paste_events.csv", show_col_types = FALSE)

count_paste_cell <- function(x) {
  if (is.na(x) || x == "[]" || x == "") return(0L)
  parsed <- tryCatch(fromJSON(x, simplifyDataFrame = TRUE), error = function(e) NULL)
  if (is.null(parsed) || nrow(parsed) == 0) return(0L)
  nrow(parsed)
}

# Take ONE row per person (the pasteEvents value is duplicated across all their rows)
person_counts <- paste_events |>
  filter(condition != "Control") |> 
  group_by(submitter_id) |>
  slice(1) |>                     # <-- key change: pick first row, don't sum across rows
  ungroup() |>
  mutate(
    q1_total = map_int(q1_pasteEvents, count_paste_cell),
    q2_total = map_int(q2_pasteEvents, count_paste_cell),
    q3_total = map_int(q3_pasteEvents, count_paste_cell),
    total_paste_events = q1_total + q2_total + q3_total
  ) |>
  select(submitter_id, condition, q1_total, q2_total, q3_total, total_paste_events)
...
person_counts |> 
  group_by(condition) |> 
  get_summary_stats(total_paste_events,
                    type = "mean_sd")


# --- Independent samples t-test ---
t_result <- t.test(total_paste_events ~ condition, data = person_counts)
t_result
```


```{r}
trial_data <- read_csv("../../data/iriss_trial_data.csv", show_col_types = FALSE)

# ============================================================
# Helpers (unchanged)
# ============================================================
parse_paste_cell <- function(x) {
  if (is.na(x) || x == "[]" || x == "") {
    return(tibble(content = character(0), finalContent = character(0),
  ...
  tibble(content = parsed$content, finalContent = parsed$finalContent, timestamp = parsed$timestamp)
}

parse_chatlog_cell <- function(x) {
  empty <- tibble(ai_message = character(0), ai_timestamp = character(0))
  if (is.na(x) || x == "[]" || x == "" || x == "NO_CHAT_DATA") return(empty)
  parsed <- tryCatch(fromJSON(x, simplifyDataFrame = TRUE), error = function(e) NULL)
  if (is.null(parsed) || length(parsed) == 0) return(empty)
  if (!is.data.frame(parsed)) parsed <- as_tibble(parsed)
  ai_rows <- parsed |> filter(type == "ai_response")
  if (nrow(ai_rows) == 0) return(empty)
  tibble(ai_message = ai_rows$content, ai_timestamp = ai_rows$timestamp)
}

# ============================================================
# Step 1: Paste content
# ============================================================
person_level_paste <- trial_data |>
  group_by(submitter_id) |>
  slice(1) |>
  ungroup() |>
  select(submitter_id, condition, q1_pasteEvents, q2_pasteEvents, q3_pasteEvents) |>
  filter(condition != "Control")

paste_content <- person_level_paste |>
  pivot_longer(cols = ends_with("pasteEvents", ignore.case = FALSE),
               names_to = "question", values_to = "raw_json") |>
  mutate(question = str_remove(question, "_pasteEvents"),
         events = map(raw_json, parse_paste_cell)) |>
  select(-raw_json) |>
  unnest(events, keep_empty = FALSE) |>
  mutate(word_count = str_count(str_trim(content), "\\S+"))

# ============================================================
# Step 2: AI chat log
# ============================================================
person_level_chat <- trial_data |>
  group_by(submitter_id) |>
  slice(1) |>
  ungroup() |>
  select(submitter_id, condition, q1_chatConversationLog, q2_chatConversationLog, q3_chatConversationLog) |>
  filter(condition != "Control")

chat_log <- person_level_chat |>
  pivot_longer(cols = ends_with("chatConversationLog", ignore.case = FALSE),
               names_to = "question", values_to = "raw_json") |>
  mutate(question = str_remove(question, "_chatConversationLog")) |>
  mutate(events = map(raw_json, parse_chatlog_cell)) |>
  select(-raw_json) |>
  unnest(events, keep_empty = FALSE)

# ============================================================
# Step 3: Combined flag — exact substring OR Jaro-Winkler similarity
# ============================================================
flag_ai_match <- function(paste_df, chat_df, jw_threshold = 0.8) {
  get_match_info <- function(text, sid, q) {
    candidates <- chat_df |> filter(submitter_id == sid, question == q)
    if (nrow(candidates) == 0 || is.na(text) || text == "") {
      return(tibble(best_match = NA_character_, similarity = NA_real_,
                     exact_substring = FALSE))
    }
    # Base R substring check -- avoids the fixed() masking issue entirely
    contains <- grepl(text, candidates$ai_message, fixed = TRUE)
    sims <- 1 - stringdist(text, candidates$ai_message, method = "jw")
    best_idx <- which.max(sims)
    tibble(
      best_match = candidates$ai_message[best_idx],
      similarity = sims[best_idx],
      exact_substring = any(contains)
    )
  }

  paste_df |>
    mutate(match_result = pmap(list(content, submitter_id, question), get_match_info)) |>
    unnest(match_result) |>
    mutate(likely_ai_paste = exact_substring | (!is.na(similarity) & similarity >= jw_threshold))
}

paste_content_flagged <- flag_ai_match(paste_content, chat_log, jw_threshold = 0.8)

# ============================================================
# Summary, by condition
# ============================================================
paste_content_flagged |>
  group_by(condition) |>
  summarise(
    n_paste_events   = n(),
    n_exact_substring = sum(exact_substring, na.rm = TRUE),
    n_jw_only        = sum(likely_ai_paste & !exact_substring, na.rm = TRUE),
    n_likely_ai      = sum(likely_ai_paste, na.rm = TRUE),
    pct_likely_ai    = mean(likely_ai_paste, na.rm = TRUE),
...
```{r}
write_csv(paste_content_flagged, "../../data/paste_content_with_ai_similarity.csv")
```
...
# Start from everyone in low/high-agency (includes people with 0 paste events)
person_summary <- person_level_paste |>
  select(submitter_id, condition) |>
  distinct() |>
  left_join(
    paste_content_flagged |>
      group_by(submitter_id) |>
  ...
  mutate(
    n_paste_events    = replace_na(n_paste_events, 0L),
    n_exact_substring = replace_na(n_exact_substring, 0L),
    prop_exact_substring = if_else(n_paste_events > 0,
                                     n_exact_substring / n_paste_events,
                                     NA_real_)
  )
...
person_summary |>
  ...
  summarise(
    n = n(),
    mean_paste_events = mean(n_paste_events), sd_paste_events = sd(n_paste_events),
    n_with_pastes = sum(!is.na(prop_exact_substring)),
    mean_prop_exact = mean(prop_exact_substring, na.rm = TRUE),
    sd_prop_exact = sd(prop_exact_substring, na.rm = TRUE)
  )

# ============================================================
# 1. Independent samples t-test: number of paste events (all people)
# ============================================================
t_paste_count <- t.test(n_paste_events ~ condition, data = person_summary)
t_paste_count

# Non-parametric check (paste counts are typically zero-inflated/skewed)
wilcox.test(n_paste_events ~ condition, data = person_summary)

# ============================================================
# 2. Independent samples t-test: proportion of exact substring matches
#    (only people who had at least one paste event)
# ============================================================
prop_data <- person_summary |> filter(!is.na(prop_exact_substring))

t_prop_exact <- t.test(prop_exact_substring ~ condition, data = prop_data)
t_prop_exact
...
```{r}
# Build a word-frequency table across ALL responses in your dataset
word_freq <- uses_normalized |>
  unnest_tokens(word, use_clean) |>
  anti_join(stop_words, by = "word") |>
  count(word, sort = TRUE) |>
  mutate(doc_freq = n / sum(n))   # relative frequency across the whole corpus

# Score each response by how rare its words are (lower avg freq = more niche)
response_rarity <- uses_normalized |>
  ...
  group_by(row_id) |>
  summarise(
    mean_word_rarity = mean(-log(doc_freq), na.rm = TRUE),  # higher = rarer words
    min_word_freq = min(doc_freq, na.rm = TRUE),
    .groups = "drop"
  ) |>
  left_join(uses_normalized |> mutate(row_id = row_number()), by = "row_id")
```
...
process_vars <- iriss_trial_data %>% 
  dplyr::select(submitter_id, 
                condition, 
                object,
                starts_with("conv_with_ai"), starts_with("consideration_aid"), starts_with("intrinsic_motivation")) %>% 
  distinct()
...
# Compute proportions for every condition × threshold
...
# Analysis from doshi_analysis.py
iriss_semantic_similarities <- read_csv("../../data/iriss_semantic_similarities.csv") 

# NOTE: Similarity, not distance
# NOTE: The number of submitters is 141 here because not all people's ideas were rated

# Set Low-Agency as the reference level
# Sets "Level_3" as the new reference baseline (Level 1)
# Set Low-Agency as the reference level
iriss_semantic_similarities <- iriss_semantic_similarities %>%
...
iriss_semantic_similarity_mod <- lmer(similarity_to_cond_obj ~ condition + (1|object) + (1|submitter_id), data=iriss_semantic_similarities)
...
evaluated_conversations <- iriss_manual_ratings_metrics %>% 
  group_by(submitter_id, rater_id, condition, object) %>% 
...
compliant_ideas <- compliant_conversations %>% 
  ...
  distinct(submitter_id, rater_id, condition, object, use)
```
...
# Get the number of unique combinations of submitter and use
compliant_ideas %>% 
  distinct(submitter_id, use) %>% 
  count(use) %>% 
  arrange(desc(n))
...
```{r}
# Get a count of how many ideas in each condition mentioned "art" or "diy"
compliant_ideas %>% 
  mutate(use = str_to_lower(use)) %>% 
  group_by(condition) %>% 
...
```{r}
# Filter to the participants whose submitter_id is in the compliant conversations dataset, then get descriptives on how long they spent on the task
iriss_trial_data %>% 
  semi_join(compliant_conversations, by="submitter_id") %>% 
  distinct(submitter_id, condition, `Duration (in seconds)`) %>%
  rename(duration = `Duration (in seconds)`) %>%
  mutate(duration = as.numeric(duration)) %>%
  # Exclude outliers who spent more than 2 hours on the task
  filter(duration < 7200) %>%
  group_by(condition) %>% 
  get_summary_stats(duration, type="mean_se") %>%
...
pairwise <- read_csv("../../data/pairwise_similarities.csv") 
```
...
# 
similarity_demographics <- left_join(pairwise, demographics_recoded, by="submitter_id")

pairwise_lmer <- lmer(similarity ~ condition + 
       scale(age) + 
       as.factor(race) + 
       as.factor(gender) + 
       (1|object) + 
       (1|submitter_id), 
     data=similarity_demographics)

summary(pairwise_lmer)
anova(pairwise_lmer)
# Conduct posthoc contrasts using joint_tests to get adjusted p-values
similarity_emmeans <- emmeans(pairwise_lmer, "condition")
similarity_posthoc <- pairs(similarity_emmeans, adjust="tukey")
print(similarity_posthoc)


```

## Doshi centroid lmer (Pre-Registered Homogeneity Analysis)
...
stargazer(iriss_doshi_mod, type = "latex", out = "tables/homogeneity.tex")
modelsummary(
  iriss_doshi_mod,
  coef_rename = c(
    "conditionHigh-Agency" = "High-Agency",
    "conditionControl" = "Control",
    "SD (Intercept submitter_id)" = "SD (Submitter intercept)",
    "SD (Intercept object)" = "SD (Object intercept)",
    "SD (Observations)" = "SD (Residual)"
  ),
  output = "tables/homogeneity.tex",
  title = "Linear Mixed Model Predicting Similarity to Condition-Object Centroid",
...
iriss_doshi %>% 
  group_by(condition) %>% 
  get_summary_stats(similarity_to_cond_obj, 
                    type="mean_sd") %>% 
  arrange(desc(mean))
...
### Similarity to Centroid by Condition
...
doshi_plot <- iriss_doshi %>% 
  ...
  #   alpha = 0.5
  # ) +
  # Add 95% CI error bars for the mean
  ...
  labs(
    x = "",
    y = "Homogeneity (Cosine Similarity to\n Condition-Object Centroid)",
    fill = "Condition"
  ) +
...
evaluations <- iriss_manual_ratings_metrics %>% 
  semi_join(compliant_ideas, by=c("submitter_id", "rater_id", "object", "use"))
```
...
# Change reference level to Low-Agency
compliant_trials_demographics <- compliant_trials_demographics %>%
  mutate(condition = fct_relevel(condition,
                                 "Low-Agency",
                                 "High-Agency",
                                 "Control"))
...
modelsummary(
  list(compliant_creativity_mod, compliant_originality_mod, compliant_usefulness_mod),
  coef_rename = c(
    "conditionHigh-Agency" = "High-Agency",
    "conditionControl" = "Control",
    "SD (Intercept submitter_id)" = "SD (Submitter intercept)",
  ...
  output = "tables/individual_metrics.tex",
  title = "Linear Mixed Models Predicting Creativity, Originality, and Usefulness",
  stars = c(`***` = 0.001, `**` = 0.01, `*` = 0.05)
)
```
...
```{r}
# Compute interrater reliability of creativity ratings
# Long data format

library(performance) # or library(psych)

# 1. Prepare long data with unit identifier
df_long <- compliant_trials_demographics %>%
  mutate(unit_id = paste(submitter_id, use, sep = "_"))

# 2. Fit a crossed random-effects model using lmer
# Creativity ~ (1 | unit_id) + (1 | rater_id)
creativiy_fit <- lmer(creativity ~ (1 | unit_id) + (1 | rater_id), data = df_long).

originality_fit <- lmer(originality ~ (1 | unit_id) + (1 | rater_id), data = df_long)

usefulness_fit <- lmer(usefulness ~ (1 | unit_id) + (1 | rater_id), data = df_long)
# 3. Extract ICC values automatically
icc(creativity_fit)
icc(originality_fit)
icc(usefulness_fit)
summary(fit)
...
```{r}
# Repeat the above analyses with an alternative model that includes random effect for rater_id

compliant_originality_aug <- lmer(
  originality ~ 
    # scale(age) + 
    # as.factor(gender) + 
    # as.factor(race) + 
    ...
    (1|object) + 
    (1|submitter_id) +
    (1|rater_id), data=compliant_trials_demographics)
summary(compliant_originality_aug)

compliant_creativity_aug <- lmer(
  creativity ~ 
    # scale(age) + 
    # as.factor(gender) + 
    # as.factor(race) + 
    ...
    (1|submitter_id) +
    (1|rater_id), data=compliant_trials_demographics)
summary(compliant_creativity_aug)

compliant_usefulness_aug <- lmer(
  usefulness ~ 
    # scale(age) + 
    # as.factor(gender) + 
    # as.factor(race) + 
...
summary(compliant_usefulness_aug)
```
...
plots <- map(outcomes, function(var) {
  summary_df <- compliant_trials_demographics %>%
    # Releveling 
    mutate(condition = fct_relevel(condition, c("Low-Agency",
                                                "High-Agency",
                                                "Control"))) %>% 
    group_by(submitter_id, condition) %>%
  ...
  ggplot(summary_df, aes(x = condition, 
                         ...
                         color = condition,
                         fill = condition)) +
    # 1. Raw data points in the background
    geom_jitter(alpha = 0.3, 
                width = 0.2, 
                size = 1.5) +
    # Add error bars for better visualization of the mean
    stat_summary(fun.data = mean_cl_boot, 
                 geom = "errorbar", 
                 width = 0.1, 
                 color = "#333333") +
        # Mean points on top (using stat_summary for calculation)
    stat_summary(fun = mean, 
                 geom = "point", 
                 shape = 21, 
                 size = 4,
                 color = "#333333") +
    ylim(c(1, 5)) +
    scale_color_manual(values = c("Control" = "#fde725FF",
                                  "Low-Agency" = "#440154FF",
                                  "High-Agency" = "#18bc9c")) +
    scale_fill_manual(values = c("Control" = "#fde725FF",
                                 "Low-Agency" = "#440154FF",
                                 "High-Agency" = "#18bc9c")) +
...
plots <- map(outcomes, function(var) {
  # Aggregate to the participant level first as in original code
  summary_df <- compliant_trials_demographics %>%
  ...
  ggplot(summary_df, aes(x = condition, 
    ...
    scale_color_manual(values = c("Control" = "#2c3e50",
                                "High-Agency" = "#18bc9c",
                                "Low-Agency" = "#e31a1c")) +
    scale_fill_manual(values = c("Control" = "#2c3e50",
                                "High-Agency" = "#18bc9c",
                                "Low-Agency" = "#e31a1c")) +
...
# These participants had ideas with > 3 creativity scores on average across their compliant trials:
...
# Which conditions were they in?
most_creative_participants %>% 
  distinct(submitter_id, condition)
...
process_vars_means <- process_vars %>% 
  # Turn all conv_, consideration_, and intrinsic_items
  # to numeric
  mutate(across(c(conv_with_ai_1:conv_with_ai_3,
                consideration_aid_1:consideration_aid_3,
              intrinsic_motivation_1:intrinsic_motivation_4), 
              as.numeric)) %>%
  group_by(submitter_id) %>% 
  mutate(conv_with_ai_mean = rowMeans(
    across(conv_with_ai_1:conv_with_ai_3),
    na.rm = TRUE
  )) %>%
  mutate(consideration_aid_mean = rowMeans(
    across(consideration_aid_1:consideration_aid_3),
    na.rm = TRUE
  )) %>%
  mutate(intrinsic_motivation_mean = rowMeans(
    across(intrinsic_motivation_1:intrinsic_motivation_4),
    na.rm = TRUE
  ))
...
process_vars_means %>% 
  group_by(condition) %>%
  get_summary_stats(conv_with_ai_mean, type="mean_sd")

process_vars_means %>% 
  group_by(condition) %>%
  get_summary_stats(consideration_aid_mean, type="mean_sd")
...
```{r}
# Omega for intrinsic motivation items
intrinsic_motivation_items <- process_vars %>% 
  select(starts_with("intrinsic_motivation_")) %>% 
  mutate(across(everything(), as.numeric))

psych::omega(intrinsic_motivation_items)

# consideration_aid_items <- process_vars %>% 
#   filter(condition != "Control") %>% 
#   select(starts_with("consideration_aid_")) %>% 
#   mutate(across(everything(), as.numeric)) %>% 
#   drop_na()
# 
# psych::omega(consideration_aid_items)

# conv_with_ai_items <- process_vars %>% 
#   filter(condition != "Control") %>% 
#   select(starts_with("conv_with_ai")) %>% 
#   mutate(across(everything(), as.numeric)) %>% 
#   distinct() %>% 
#   drop_na()
# 
# psych::omega(conv_with_ai_items)
```
...
duration %>% 
  group_by(condition) %>%
  get_summary_stats(duration_min, type = "mean_se")
...
# Merge direct asks with individual-level creativity
direct_asks_creativity <- direct_asks %>% 
  left_join(submitter_creativity, 
            by=c("submitter_id", "condition"))

# See if direct_asks predict creativity
direct_ask_creativity_mod <- lm(creativity ~
                                  direct_ask_proportion,
                                data =
                                  direct_asks_creativity)
summary(direct_ask_creativity_mod)

ggplot(direct_asks_creativity,
       aes(x=condition,
           color = direct_ask_proportion,
           y = creativity)) +
  geom_point(position = 
...
```{r}
cluster_stats <- read_csv("data/cluster_stats_summary.csv")
```

```{r}
# Do a one-way ANOVA to test for differences in number of clusters between conditions
cluster_anova <- aov(Unique_Clusters ~ Condition, data=cluster_stats)
summary(cluster_anova)

noise_ratio <- aov(Noise_Ratio ~ Condition, data=cluster_stats)
summary(noise_ratio)

coef_variation <- aov(Coefficient_of_Variation ~ Condition, data=cluster_stats)
summary(coef_variation)

# Tukey's HSD post-hoc test to see which conditions differ
cluster_tukey <- TukeyHSD(cluster_anova)
cluster_tukey

noise_ratio_tukey <- TukeyHSD(noise_ratio)
noise_ratio_tukey

coef_variation_tukey <- TukeyHSD(coef_variation)
coef_variation_tukey
```
