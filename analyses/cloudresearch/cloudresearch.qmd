...
qualtrics_api_credentials(
  api_key = "LeAkzh7rVjF9cUNbcpIn8aUbUGnt4IntptFvlrOq",
  base_url = "stanforduniversity.sjc1.qualtrics.com", 
  install = TRUE,
  overwrite = TRUE
)

# Force R to reload the new credentials
...
write_csv(all_ratings_demographics, "data/cloudresearch_ideas.csv")
...
# Contrasts, with factors in alphabetical order
contrast = list(
  "high_vs_low" = c(0, 1, -1, 0)
)
...
```{r}
# Check the mean completion time across conditions
cleaned_names %>%
  group_by(Condition) %>% 
  get_summary_stats(`Duration (in seconds)`, type="mean_se") %>% 
  ggplot(aes(x=Condition, y=mean, color=Condition)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se)) +
  labs(y="Mean Completion Time (seconds)",
       title = "CloudResearch") +
  scale_color_manual(values = c("control" = "#619bff", "high-agency" = "#00BA38", "low-agency" = "#F8766D", "unstructured" = "gold")) +
  ylim(c(0, 2000)) +
  jtools::theme_apa() +
  theme(legend.position = "none")
```
High-Agency and Unstructured took the longest time to complete the survey. Control participants were the fastest, which is what would be expected. 
