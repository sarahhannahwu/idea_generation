...
high-agency_data <- read_csv("data/high-agency_2025.09.11.csv") %>% 
  slice(-(1:2))

# Filter for participants who passed attention check
passed_attn_check <- high-agency_data %>% 
...
high-agency_data_renamed <- passed_attn_check %>% 
...
trial_data <- high-agency_data_renamed %>% 
...
submitter_demographics <- iriss %>%
  ...
  select(submitter_id, age, gender, race) 
...
low-agencys produced the most, followed by high-agencys, then participants in the control condition.
...
```{r}
no_chats_high-agency <- read_csv("data/high-agency_no_first_message.csv")

no_chats_low-agency <- read_csv("data/low-agency_no_first_message.csv")

noncompliant_conversations <- bind_rows(no_chats_high-agency, no_chats_low-agency) %>% 
