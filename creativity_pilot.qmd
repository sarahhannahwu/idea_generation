...
self_perception_plots <- map(self_perception_outcomes, function(var) {
  ...
  ggplot(summary_df, aes(x = value, y = condition, fill = condition)) +
    ...
    xlim(c(1,5)) +
    labs(x = var, y = "condition") +
    theme_apa() +
    theme(legend.position = "none")
})

# To display the plots:
self_perception_plots[[1]]  # originality
self_perception_plots[[2]]  # creativity
self_perception_plots[[3]]  # usefulness
...
iriss_self_creativity_mod <- lmer(
  creativity ~ condition + (1|object) + (1|ResponseId), data= iriss_self_perceptions)
summary(iriss_self_creativity_mod)

iriss_self_usefulness_mod <- lmer(
  usefulness ~ condition + (1|object) + (1|ResponseId), data= iriss_self_perceptions)
summary(iriss_self_usefulness_mod)
```
Pilots perceived their ideas as more useful than the other participants. 
...
process_vars <- iriss_trial_data %>% 
  rename(submitter_id = ResponseId) %>% 
  select(submitter_id, object, use, starts_with("conv_with_ai"), starts_with("consideration_aid"), starts_with("intrinsic_motivation"))
...
ggplot(iriss_manual_ratings_metrics, aes(x=originality, y=usefulness, fill=as.factor(creativity))) +
...
Passengers produced the most, followed by Pilots, then participants in the control condition.
...
```{r}
no_chats_pilot <- read_csv("data/pilot_no_first_message.csv")

no_chats_passenger <- read_csv("data/passenger_no_first_message.csv")

noncompliant <- bind_rows(no_chats_pilot, no_chats_passenger) %>% 
...
# Exclude the noncompliant trials from the analysis
...
compliant_originality_mod <- lmer(
  originality ~ condition + (1|object) + (1|submitter_id), data= compliant_trials)
summary(compliant_originality_mod)   

compliant_creativity_mod <- lmer(
  creativity ~ condition + (1|object) + (1|submitter_id), data= compliant_trials)
summary(compliant_creativity_mod)   

compliant_usefulness_mod <- lmer(
  usefulness ~ condition + (1|object) + (1|submitter_id), data= compliant_trials)
summary(compliant_usefulness_mod)  

compliant_similarities <- iriss_semantic_similarities %>% 
  anti_join(noncompliant, by=c("submitter_id", "object"))

compliant_semantic_similarity_mod <- lmer(similarity ~ condition + (1|object) + (1|submitter_id), data=compliant_similarities)
summary(compliant_semantic_similarity_mod)
...
```
Around 24% of the Passenger conversations did not use AI and around 38% of the Pilot conversations did not use AI. Excluding the noncompliant trials did not change the results.
...
summary(onefac4items, standardized=TRUE)
```
